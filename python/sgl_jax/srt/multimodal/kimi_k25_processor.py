...
class KimiK25Processor(ProcessorMixin):
    ...
    def __call__(self,
         ...
         text: str = None,
         images: list = None,   # ← add: PIL images from multimodal_tokenizer
         videos: list = None,   # ← add: video frames/paths
         audio: list = None,    # ← add: ignored for now
        ...
        """
        # Convert images/videos kwargs (Qwen-style API) to medias format
        if medias is None and (images is not None or videos is not None):
            medias = []
            if images:
                medias.extend([{'type': 'image', 'image': img} for img in images])
            if videos:
                medias.extend([{'type': 'video', 'video': vid} for vid in videos])
        print(messages, medias, text)

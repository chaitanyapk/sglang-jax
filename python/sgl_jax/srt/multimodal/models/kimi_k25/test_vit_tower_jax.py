...
def make_pixel_values(
    grid_thws: list[tuple[int, int, int]], rng: jax.Array, dtype=jnp.bfloat16
) -> jnp.ndarray:
    """
...
def normalise_output(output: list[jnp.ndarray] | jnp.ndarray) -> list[np.ndarray]:
    """
...
def check_value_range(arr: np.ndarray, label: str, lo: float = -1e4, hi: float = 1e4) -> bool:
    f32 = arr.astype(np.float32)
    if f32.min() < lo or f32.max() > hi:
        print(
            f"  WARN [{label}]: values outside [{lo}, {hi}]: "
...
def run_test_case(
    model: Kimi_K25_VisionModel, tc: TestCase, rng: jax.Array, verbose: bool = True
) -> bool:
    ...
    if len(outputs) != len(tc.grid_thws):
        print(
            f"  FAIL [{tc.name}]: expected {len(tc.grid_thws)} output tensors, "
            f"got {len(outputs)}"
        )
        return False
    ...
    for i, (arr, exp_shape) in enumerate(zip(outputs, expected_shapes)):
        ...
        if verbose:
            ...
            print(
                f"  {status} img[{i}]: shape={arr.shape} dtype={arr.dtype} "
...
def run_determinism_test(model: Kimi_K25_VisionModel, rng: jax.Array) -> bool:
    print(f"\n{'='*60}")
...
def run_reference_comparison(
    model: Kimi_K25_VisionModel, ref_npz_path: str, atol: float = 0.02, rtol: float = 0.01
) -> bool:
    """
    ...
    while f"output_{i}" in data:
        ...
        status = "PASS" if ok else "FAIL"
        print(
            f"  {status} [{label}]: max_abs_err={max_abs_err:.6f} (atol={atol})  "
...
def parse_args():
    ...
    p.add_argument(
        "--reference",
        default=None,
        help="Path to a .npz file with reference outputs for numerical " "comparison (optional).",
    )
    p.add_argument(
        "--atol",
        type=float,
        default=0.02,
        help="Absolute tolerance for reference comparison (default 0.02 " "for bfloat16).",
    )
    p.add_argument(
        "--rtol", type=float, default=0.01, help="Relative tolerance for reference comparison."
    )
    p.add_argument("--seed", type=int, default=42, help="JAX PRNG seed for synthetic inputs.")

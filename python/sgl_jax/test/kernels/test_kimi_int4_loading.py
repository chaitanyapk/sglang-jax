...
def main():
    ...
    # Check if custom number of layers is requested via environment variable
    import os
    ...
    if num_layers_env is not None:
        ...
        logger.info("Using default number of layers from config (full model execution)...")
    ...
    # Access an expert from Layer 1
    layers = model.model.layers
    moe_layer = layers[1]  # DeepSeek v3 MoE layers typically start at layer 1
    
    if hasattr(moe_layer.mlp, "wi_0"):
        param_before = moe_layer.mlp.wi_0.value
        logger.info("Abstract parameter type before loading: %s", type(param_before))
        assert isinstance(param_before, jax.ShapeDtypeStruct), "Weight is not abstract before load!"
        logger.info("Abstract Parameter ShapeDtypeStruct: shape=%s, dtype=%s", param_before.shape, param_before.dtype)
    
    # 5. Load weights via WeightLoader
    # WeightLoader reads safetensors from disk, and shards/materializes them directly onto TPU slice-by-slice
    ...
    if hasattr(moe_layer.mlp, "wi_0"):
        ...
        # HBM (TPU) Stage Logger details
        logger.info("HBM (TPU) Parameter detailed allocation - dtype: %s, sharding: %s, memory_kind: %s", 
                    param_after.dtype, param_after.sharding, param_after.sharding.memory_kind)

        # Compute stats to make sure they are successfully populated with non-zero values from safetensors
        mean_val = jnp.mean(param_after.astype(jnp.float32)).item()
        logger.info("Loaded expert wi_0 mean weight value: %f", mean_val)

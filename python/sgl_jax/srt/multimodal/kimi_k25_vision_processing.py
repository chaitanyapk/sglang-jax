...
from .media_utils import (MediaInput, VideoChunkInput,
    ...
    def preprocess(
        ...
        if medias:
            ...
            pixel_values = np.concatenate([  
                np.array(pixel_value['pixel_values'])  
                for pixel_value in normalized_pixel_values  
            ])  
            grid_thws = np.concatenate([  
                np.expand_dims(np.array(pixel_value['grid_thw'], dtype=np.int64), 0)  
                for pixel_value in normalized_pixel_values  
            ])
